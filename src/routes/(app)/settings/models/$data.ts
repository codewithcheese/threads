...
export type AccountsView = Awaited<ReturnType<typeof loadAccounts>>;

export async function loadAccounts() {
  return useDb().query.accountTable.findMany({
    with: {
      models: true,
    },
  });
...
export async function toggleVisible(service: Service, model: Model) {
  await useDb()
    ...
    .where(
      and(eq(modelTable.id, model.id), eq(modelTable.serviceId, service.id)),
    );
  await invalidateModel(serviceTable, service);
